...
from src.loader import Custom_CIFAR10, Custom_Binary_CIFAR10
...
def binary_train(model, configs, logger, cls, device='cuda'):
    model.train()
    ...
    train_datasets = Custom_Binary_CIFAR10(root_path=configs['root_path'],
    ...
    logger.info(f"[TRAIN Acc / {train_acc}] [TRAIN Loss / {train_loss}] [Airplane / {n_airplane}]" )

    return model


def binary_test(model, configs, logger, cls, device='cuda'):
    model.eval()
    transformer = transforms.Compose([transforms.ToTensor(),
                                      transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))])

    test_datasets = Custom_Binary_CIFAR10(root_path=configs['root_path'],
                                          class_names=['airplane', 'dog', 'horse'],
                                          cls=cls,
                                          dtype='test',
                                          transformer=transformer)
...
def train(model, configs, logger, device='cuda'):
    ...
    for i, (images, labels) in tqdm(enumerate(train_loader), total=train_iter):
        ...
        n_airplane += (predicted == 0).sum().item()
        n_train_correct += (predicted == labels).sum().item()
        # loss
        loss = criterion(pred, labels)
        train_loss += loss.item()
