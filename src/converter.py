...
def cvt_linear(linear, post, device='cuda'):
    new_linear = torch.nn.Linear(in_features=16*16*len(post),
    ...
    for i in post:
        weights.append(old_weights[:, 16*16*i:16*16*(i+1)])

    new_weights = np.concatenate((weights), axis=1)
    new_weights = np.array(new_weights)

    new_linear.weight.data = torch.from_numpy(new_weights)
    new_linear.weight.data = new_linear.weight.data.cuda()

    new_linear.bias.data = linear.bias.data.cuda()
